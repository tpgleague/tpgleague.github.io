...
function smarty_function_quickform_fieldset($params, &$smarty)
{
		...
		//add fieldset's notes box, which we know from the ID.
		if ( isset($notes_label) && $notes_label!='' )
		{
			$html .= "\t<div class=\"notes\">\n";
			$html .= "\t<div class=\"notes_label\">".$notes_label."</div>\n";
			$html .= "\t".$notes;
			$html .= "\t</div>\n";
		}
        ...
        for($i = 0; $i < count($fields); $i++)
        {
                ...
                else
                {
					...
					$labelfor = ' for="'.$el['name'].'"';
					if ($el['type'] == 'group' || $el['type'] == 'date') $labelfor = '';
					$el['html'] = "<label$labelfor>".$el['label'].'</label>'.$el['html'].'';
                }
                ...
                if ( $el['error'] ) { $divclass[] = 'error'; $diverrormsg = '<p class="error">'.$el['error'].'</p>'; }
				if ( 'checkbox' == $el['type']) $divclass[] = 'checkbox';
                if (! empty($divclass) ) $class_text = ' class="'.implode(' ', $divclass).'"';
				if ( isset($form['note_'.$el['name']]) ) $note = '<div class="small_note">'.$form['note_'.$el['name']]['label'].'</div>';
                //assemble the html string for this element
                $html .= "\n\t<div".$class_text.'>'.$diverrormsg.$el['html'].$note."\n\t</div>";
