...
//----------------------------------------------------------------------------------------------------
    ...
    function build_legend($barColors) {
      ...
      for($i = 0; $i < count($barColors); $i++) {
        ...
        $legend .= '<td style="' . $this->cssLEGEND . '" nowrap="nowrap">' . trim($l[$i]) . '</td>';
        $legend .= '</tr>';
      }
    ...
    function create_hBar($value, $percent, $mPerc, $mPerc_neg, $max_neg, $mul, $valSpace, $bColor, $border, $spacer, $spacer_neg) {
      $bar = '<table border="0" cellspacing="0" cellpadding="0" style="height: 100%"><tr>';
      ...
      if($percent < 0) {
        ...
        else $bar .= '<td><div style="display: block; width: 1px; height: '. ($this->barWidth + ($border * 2)) .'px"></div></td>'; //else $bar .= '<td><img width="1" height="' . ($this->barWidth + ($border * 2)) . '" /></td>';
    ...
    function create_vBar($value, $percent, $mPerc, $mPerc_neg, $max_neg, $mul, $valSpace, $bColor, $border, $spacer, $spacer_neg) {
      ...
      if($percent < 0) {
        ...
        else $bar .= '</tr><tr><td><div style="display: block; width: '. ($this->barWidth + ($border * 2)) .'px; height: 1px;"></div></td>'; //<img width="' . ($this->barWidth + ($border * 2)) . '" height="1" /></td>';
        if($max_neg) {
    ...
    function create() {
      ...
      $graph = '<table style="margin: 0 auto;" border="0" cellspacing="0" cellpadding="' . $this->graphPadding . '" class="table_poll"><tr>';
      ...
      for($chart = $lcnt = 0; $chart < count($val); $chart++) {
        ...
        if($this->type == 'hbar') {
          for($i = 0; $i < count($val[$chart]); $i++, $lcnt++) {
            ...
            $graph .= '<tr><td style="' . $this->cssLABEL . '" width="65%" ' . (($rowspan > 1) ? ' rowspan="' . $rowspan . '"': '') . ' align="left">';
            $graph .= '&nbsp;' . $label . '&nbsp;</td>';
            ...
            for($j = 0; $j < count($val[$chart][$i]); $j++) {
              ...
              $graph .= '<td' . ($this->cssBARBG ? ' style="' . $this->cssBARBG . '"' : '') . ' height="100%" width="35%">';
          ...
          for($i = 0; $i < count($val[$chart]); $i++, $lcnt++) {
            ...
            $graph .= '</tr></table></td>';
            if($this->showValues < 2) $graph .= '<td style="' . $this->cssPERCVALUES . '" nowrap="nowrap">&nbsp;' . number_format($percent, $this->percValuesDecimals) . '%</td>';
            $graph .= '</tr>';
