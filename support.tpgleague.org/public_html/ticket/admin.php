...
// ST4NK! Ticket Beta v0.1 - admin.php
...
print "<table align=center bgcolor=#FFFFFF border=0 cellpadding=2 cellspacing=2 width=1000>\n";

print "<tr><td colspan=8><p><i>You are logged in as {$_SESSION["loginUsername"]} </i></p></td></tr>\n";

// SHOW PENDING (UNASSIGNED) TICKETS

print "<tr><td colspan=8>
			<form action=admin.ticket_global_sort.php method=post name=form>
			<input type=hidden name=sort_table value=pending>
			<p>Sort by 
...
print "<tr><td colspan=9><form action=admin.ticket_summary.php method=get>
	...
	&nbsp &nbsp Short cut to <a class=two href=admin.ticket_global.php>view all tickets</a></p></form></td></tr>";

print "<tr><td colspan=9><p><b>Pending Tickets</p></td></tr>\n";

print "<tr>
		<td bgcolor=#999999 width=50><p class=ticketheader><b>ID</b></td>
		<td bgcolor=#999999 width=150><p class=ticketheader><b>Email</b></td>
		<td bgcolor=#999999 width=150><p class=ticketheader><b>Catagory</b></td>
		<td bgcolor=#999999 width=250><p class=ticketheader><b>Subject</b></td>
		<td bgcolor=#999999 width=100><p class=ticketheader><b></b></td>
		<td bgcolor=#999999 width=200><p class=ticketheader><b>Created</b></td>
		<td bgcolor=#999999 width=100><p class=ticketheader><b></b></td>
	</tr>\n";

$connection = mysql_connect($hostName, $username, $password);
mysql_select_db($databaseName, $connection);
$result = mysql_query ("SELECT * FROM st4nkticket_tickets WHERE ticket_helper = 0 ORDER BY $sort_column $sort_order");
...

	{
		...
		print "<tr>
				<td bgcolor=#999999><p class=ticketheader><a class=one href=admin.ticket_view.php?ticket_id=$id>$id</a></p></td>
				...
				<$td><p class=ticketheader><a class=two href=admin.ticket_view.php?ticket_id=$id>View Ticket</a></p></td>
			</tr>\n";
	}
...
$connection = mysql_connect($hostName, $username, $password);
mysql_select_db($databaseName, $connection);
$num = mysql_query ("SELECT * FROM st4nkticket_tickets WHERE ticket_helper = '$ticket_helper'");
...
print "<tr><td colspan=9><p><b>My Tickets</p></td></tr>\n";

print "<tr>
		<td bgcolor=#999999><p class=ticketheader><b>ID</b></td>
		<td bgcolor=#999999><p class=ticketheader><b>Email</b></td>
		<td bgcolor=#999999><p class=ticketheader><b>Catagory</b></td>
		<td bgcolor=#999999><p class=ticketheader><b>Subject</b></td>
		<td bgcolor=#999999><p class=ticketheader><b></b></td>
		<td bgcolor=#999999><p class=ticketheader><b>Last Update</b></td>
		<td bgcolor=#999999><p class=ticketheader><b>Status</b></td>
	</tr>\n";

$connection = mysql_connect($hostName, $username, $password);
mysql_select_db($databaseName, $connection);
$result = mysql_query ("SELECT * FROM st4nkticket_tickets WHERE ticket_helper = '$ticket_helper' ORDER BY $sort_column $sort_order LIMIT ".$offset.",".$rows_page."");
...
while ($row = mysql_fetch_array($result))

	{
		...
		$date_updated = gmdate("D M d y g:i a", $timestamp_updated + 3600 * $timeoffset);

		// DETERMINE COLOR OF ROW BASED ON TICKET AND UPDATED BY STATUS
...
{
	print "<tr><td colspan=8><p></p></td></tr>";
	print "<tr><td colspan=8><p>Displaying $rows of $rows tickets</td></tr>";
}
...

{
	...
	print "<tr><td colspan=9><p> << First | < Prev | <a class=two href=admin.php?offset=".$next."> Next > </a> | <a class=two href=admin.php?offset=".$end."> Last >> </a>";
...
{	
	...
	$begin = $offset +1;
	print "<tr><td colspan=9><p></p></td></tr>";
	print "<tr><td colspan=9><p><a class=two href=admin.php?offset=0> << First </a> | <a class=two href=admin.php?offset=".$prev."> < Prev </a> | Next > </a> | End >> ";
...

{
	...
	$end = floor($rows/$rows_page) * $rows_page;
	print "<tr><td colspan=9><p></p></td></tr>";
	print "<tr><td colspan=9><p><a class=two href=admin.php?offset=0> << First </a> | <a class=two href=admin.php?offset=".$prev."> < Prev </a> | <a class=two href=admin.php?offset=".$next."> Next > </a> | <a class=two href=admin.php?offset=".$end."> End >> </a>";
	print "<tr><td colspan=9><p>Displaying $begin - $next of $rows tickets</td></tr>";	
}
